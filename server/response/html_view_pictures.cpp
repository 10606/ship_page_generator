...
"<script>\n"
    ...
    "{\n"
        ...
        "if (tag_name == \"DIV\")\n"
            "picture_li = event.target.nextElementSibling.childNodes;\n"
        "if (tag_name == \"LI\")\n"
            "picture_li = event.target.parentNode.parentNode.childNodes;\n"
        "var i = 0;\n"
        "for (picture of picture_li)\n"
        "{\n"
            "if (picture.tagName != \"LI\" || picture.childNodes.length < 3)\n"
                "continue;\n" // text that just new line, skip it
            "if (picture == event.target.parentNode)\n"
                "picture_index = i;\n"
            "var link = picture.childNodes[0];\n"
            "var description = String();\n"

            "for (var j = 0; j != picture.childNodes.length; ++j)\n"
